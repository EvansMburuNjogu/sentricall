...
class SessionActivity : AppCompatActivity(), ScreenRecordingService.RecordingCallback {
    ...
    companion object {
        ...
        // when launched from GuardFragment we pass this
    ...
    private var sessionId: String? = null   // 🔹 set from Intent, used for CRUD / AI later
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Session id (for CRUD / conversations)
        ...
        // 🔹 Refresh session details from backend (name, etc.) if we have an id
    ...
    /**
     * If we know the backend session id, ask the API for latest details.
     * Currently we only refresh the name -> header title.
     * All recording / AI logic stays unchanged.
     */
        ...
        Thread {
            ...
            try {
                ...
                if (status !in 200..299) {
                    // Swallow errors silently for now – header text from Intent is still fine
                    return@Thread
                }
                ...
                val name = sessionObj.optString("name", "")

                if (name.isNotBlank()) {
                    runOnUiThread {
                        // Only update title; subtitle & mode are kept from Intent
                        tvSessionHeaderTitle.text = name
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
                // Ignore failure – UI continues with existing title
            } finally {
                conn?.disconnect()
            }
        }.start()
    ...
    private fun showReportDialog() {
        ...
        btnSubmit.setOnClickListener {
            ...
            // TODO: send to backend (can also include sessionId & markSuspicious)
